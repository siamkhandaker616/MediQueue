<x-layouts.staff>
    ...
    @php
        $selectedRange = request()->query('range', 'week');
    ...
    <div class="mb-6 flex flex-wrap items-end justify-between gap-3">
        ...
        <div class="flex flex-wrap items-center gap-2">
            <a href="{{ route('admin.analytics.export', request()->query()) }}" class="btn-outline">Export CSV</a>
            <a href="{{ route('admin.analytics.print', request()->query()) }}" class="btn-primary">Print report</a>
        </div>
    </div>

    <form method="GET" action="{{ route('admin.analytics') }}" class="card p-4">
        <div class="flex flex-wrap items-end gap-3">
            <div class="flex rounded-lg bg-brand-50 p-1">
                @foreach (['today' => 'Today', 'week' => 'This week', 'month' => 'This month'] as $value => $label)
                    <a href="{{ route('admin.analytics', ['range' => $value]) }}"
                       class="rounded-md px-3 py-1.5 text-sm font-medium {{ $selectedRange === $value ? 'bg-brand-600 text-white' : 'text-brand-700 hover:bg-brand-100' }}">
                        {{ $label }}
                    </a>
                @endforeach
            </div>
            ...
            <div class="flex items-end gap-2">
                <div>
                    ...
                    <input id="from" name="from" type="date" value="{{ $from }}" class="input">
                </div>
                <div>
                    <label for="to" class="label">To</label>
                    <input id="to" name="to" type="date" value="{{ $to }}" class="input">
                </div>
    ...
    <div class="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div class="stat-card">
            <p class="text-sm font-medium text-muted">Appointments</p>
            <p class="mt-1 text-3xl font-bold text-brand-600">{{ $appointments }}</p>
            <p class="text-xs text-muted">{{ $rangeLabel }}</p>
        </div>
        <div class="stat-card">
            <p class="text-sm font-medium text-muted">Revenue</p>
    ...
    <div class="mt-6 grid gap-6 lg:grid-cols-2">
        <div class="card p-5">
            <h2 class="font-semibold">Revenue by method</h2>
            @if ($revenueByMethod->isEmpty())
                <p class="mt-10 text-center text-sm text-muted">No payments in this period.</p>
            @else
                <div class="mt-4 h-64"><canvas id="methodChart"></canvas></div>
            @endif
        </div>
        ...
        <div class="card p-5">
            ...
            <div class="mt-4 grid grid-cols-3 gap-3">
                <div class="rounded-xl bg-red-50 p-4 text-center">
                    <p class="text-2xl font-bold text-red-600">{{ $refundCount }}</p>
                    <p class="mt-1 text-xs text-muted">refunded</p>
                </div>
                <div class="rounded-xl bg-red-50 p-4 text-center">
                    <p class="text-2xl font-bold text-red-600">৳{{ number_format($refundAmount) }}</p>
                    <p class="mt-1 text-xs text-muted">refunded amount</p>
                </div>
                <div class="rounded-xl bg-red-50 p-4 text-center">
                    <p class="text-2xl font-bold text-red-600">{{ $refundRate }}%</p>
                    <p class="mt-1 text-xs text-muted">refund rate</p>
                </div>
            </div>
        </div>
    </div>
            ...
            @if ($revenueByMethod->isNotEmpty())
                new Chart(document.getElementById('methodChart'), {
                    ...
                    data: {
                        ...
                        datasets: [{
                            data: data.methods.map(m => m.total),
                            backgroundColor: [rgb(brand), rgb(accent), rgb(brand2), rgb('252 211 77')],
                            borderWidth: 0,
                        }],
                    },
                    options: { plugins: { legend: { position: 'right' } }, maintainAspectRatio: false },
